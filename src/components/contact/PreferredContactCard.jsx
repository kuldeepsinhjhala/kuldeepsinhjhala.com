/**
 * PreferredContactCard - Displays a preferred contact method
 */
function PreferredContactCard({ method = {}, contact = {}, social = {} }) {
  if (!method || !method.method) return null
  ...
  const getContactLink = () => {
    switch (method.method) {
      case 'email':
        return contact.email?.primary ? `mailto:${contact.email.primary}` : null
      case 'linkedin':
        return social.linkedin?.url || null
      case 'whatsapp':
        return contact.phone?.whatsapp || null
      default:
        return null
    }
  }
  ...
  const getContactText = () => {
    switch (method.method) {
      case 'email':
        return contact.email?.primary || 'Email'
      case 'linkedin':
        return social.linkedin?.username ? `@${social.linkedin.username}` : 'LinkedIn'
      case 'whatsapp':
        return contact.phone?.primary || 'WhatsApp'
      default:
  ...
  return (
    ...
    >
      <div className="text-gold mb-4">
        {getIcon(method.icon || method.method)}
      </div>
      ...
      {method.description && (
        <p className="text-body text-sm md:text-base mb-4 leading-relaxed flex-1">
          {method.description}
        </p>
      )}
...
export default PreferredContactCard
